...
<style>
	*{ margin: 0; padding: 0; box-sizing: border-box;}
	...
	.mainDiv{ margin: auto; width: 250px; text-align: center; padding: 40px 0; }
...
<body>
	...
	<div class="mainDiv">
		...
		<c:if test="${not empty registered}"><div class="success">${registered}</div><h3 class="spaceBetweenInput"></h3></c:if>
		<c:if test="${not empty logoutSuccess}"><div class="success">${logoutSuccess}</div><h3 class="spaceBetweenInput"></h3></c:if>
		<c:if test="${not empty error}"><div class="error">${error}</div><h3 class="spaceBetweenInput"></h3></c:if>
		<form name='login' action="<c:url value='/login' />" method='POST'>
			<p>Username:</p> <input class="loginInput inline" type='text' name='username' value=''>
			<h3 class="spaceBetweenInput"></h3>
			<p>Password:</p>  <input class="loginInput inline" type='password' name='password'>
			<h3 class="spaceBetweenInput"></h3>
